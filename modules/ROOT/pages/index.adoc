...
Esta sección establece los fundamentos técnicos y estándares que respaldan las configuraciones implementadas en la plataforma de datos IoT de San Gabriel. Su propósito es validar que las prácticas aplicadas están alineadas con estándares reconocidos y facilitar la replicación de la solución en futuras implementaciones.
...
Ignition utiliza asegura un uso eficiente de la red y actualizaciones oportunas de los tags. Una vez recibidos, los datos se reorganizan en una estructura jerárquica basada en ISA-95: *Cempro → San Gabriel → Clinkerización → Equipos específicos*. Adicionalmente, implementa **Store and Forward**, guardando datos localmente en disco cuando hay pérdida de conectividad de red.
